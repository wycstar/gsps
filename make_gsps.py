...
def generate(input_file, json_file, output_file):
    input_dicom = pydicom.read_file(input_file)
    dataset = Dataset()
    dataset = __make_file(output_file)
...
def __img_object(input_dicom):
    ...
    img.SOPInstanceUID = pydicom.uid.generate_uid()
    return img
...
def __copy_from_original(dataset, input_dicom):
    data_elements = [
        ...
        "Accession Number", "Series Instance UID", "Manufacturer", "Specific Character Set"
    ]
    for de in input_dicom:
        if de.name in data_elements:
            dataset.add(de)
    dataset.InstanceCreationDate = time.strftime("%Y%m%d", time.localtime(time.time()))
    dataset.InstanceCreationTime = time.strftime("%H%M%S", time.localtime(time.time()))
    return dataset
...
def __add_annotation(dataset, json_file, input_dicom):
    ...
    for layer, circles in annotation_data.items():
        layer_objects.append(
            __add_layer(dataset, {layer: circles}, input_dicom))
...
def __add_layer(dataset, layer, input_dicom):
    ...
    layer_object.GraphicLayer = list(layer.keys())[0]
    img = __img_object(input_dicom)
    layer_object.ReferencedImageSequence = Sequence([img])
    layer_object = __add_circles(layer_object, list(layer.values())[0])
    return layer_object
...
def __add_display_area(dataset, input_dicom):
    ...
    ds_displayed_area_selection.DisplayedAreaBottomRightHandCorner = [
        input_dicom.Columns, input_dicom.Rows
    ]
    ...
    dataset.DisplayedAreaSelectionSequence = Sequence(
        [ds_displayed_area_selection])
    return dataset


def __make_file(output_file):
...
def test():
    __make_sample()
    dcm = generate(input_file="./images/sample1.dcm",
